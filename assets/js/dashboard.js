...
function renderServices(locks={}){const general=locks.general_lock||{},legacy=locks.services_lock||{};const available=services.filter(([key])=>!off(Object.hasOwn(general,key)?general[key]:legacy[key]));document.querySelector('#service-grid').innerHTML=available.length?available.map(([key,icon,label])=>`<a class="service" href="service.html?service=${key}"><i class="ico bi bi-${icon}"></i>${label}</a>`).join(''):'<div class="empty">No service is currently available.</div>'}
renderServices();
